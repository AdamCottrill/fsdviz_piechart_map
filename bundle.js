(function () {
	...
	const column = "count";
	const filters = {};
	tsv$1("data/data.tsv", prepare_data).then(data => {
	  ...
	  let colorGroup = colorDim.group().reduceSum(d => d[column]); //ininitialize our filters - all checked at first

	  filters["color"] = colorDim.group().all().map(d => d.key);
	  filters["category"] = categoryDim.group().all().map(d => d.key);
	  filters["group"] = groupDim.group().all().map(d => d.key);
